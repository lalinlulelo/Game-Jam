...
public class moverLuna : MonoBehaviour
{
    ...
    void Update()
    {
        //modificar el segundo parametro hace que se prolongue horizontalmente, el primero verticalmente
        transform.position = new Vector2((xInicio + (primero * MCos(alpha) * MSin(tilt)) + (segundo * MSin(alpha) * MCos(tilt))),
            (yInicio + (tercero * MCos(alpha) * MCos(tilt)) - (cuarto * MSin(alpha) * MSin(tilt))));
